...
class User {
	protected static  $tblname = "tbluseraccounts";
    ...
    function userAuthentication($U_USERNAME,$h_pass){
		global $mydb;
		$mydb->setQuery("SELECT * FROM `tbluseraccounts` WHERE `USERNAME` = '". $U_USERNAME ."' and `PASS` = '". $h_pass ."'");
		$cur = $mydb->executeQuery();
		if($cur==false){
			die(mysql_error());
		}
		$row_count = $mydb->num_rows($cur);//get the number of count
		 ...
		 $user_found = $mydb->loadSingleResult();
		 	...
		 	$_SESSION['UROLE'] 			= $user_found->UROLE;
		   return true;
		 }else{
		 	return false;
		 }
