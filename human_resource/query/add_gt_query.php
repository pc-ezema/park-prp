...
if(isset($_POST['save'])) {
	...
	if(in_array($end, $allowed_ext)){
	   ...
	   $add = $con->prepare("INSERT INTO tblgt (CODE, SNAME, FNAME, MNAME, DateofBirth, Religion, PlaceofBirth, Sex, Nationality, StateofOrigin, LGA, HomePlace, 
							RADDRESS, TELNO, Email, MaritalStatus, HealthLimitations, BloodGroup, Geneotype, LevelEducation, InstitutionAttended, CourseofStudy, Duration,
							...
							KinName, KinRelationship, KinAddress, KinPhoneNumber, photo) 
							VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		$add->execute(array($CODE, $SNAME, $FNAME, $MNAME, $DateofBirth, $Religion, $PlaceofBirth, $Sex, $Nationality, $StateofOrigin, $LGA, $HomePlace, $RADDRESS,
						...
						$TrainingExpectation, $CompanyName, $CompanyAddress, $Designation, $NameofSupervisor, $PhoneNumberSupervisor, $AccountName, $AccountNumber, 
						$KinName, $KinRelationship, $KinAddress, $KinPhoneNumber, $path));
