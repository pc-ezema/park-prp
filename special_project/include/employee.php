...
class User{
	...
	protected static $tbl_name = "tblaccounts";
	function db_fields(){
		global $mydb;
	...
	function find_all_user($name=""){
	 ...
	 static function AuthenticateUser($email="", $h_upass=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM `tblaccounts` WHERE `ACCOUNT_USERNAME`='" . $email . "' and `ACCOUNT_PASSWORD`='" . $h_upass ."' LIMIT 1");
