...
if(isset($_POST['save'])) {
	...
	$mail->Body    = 'Complain Report has been sent to you, visit your Dashboard for more details.';

	$mail->send();
			
	$add_report = $con->prepare("INSERT INTO tblcompliancefile (department, date, name, file) VALUES (?,?,?,?)");
	$add_report->execute(array($Department, $Date, $name[0], $path));
	
	move_uploaded_file($file_temp, $path);
	header('location:../all_complain.php');
	}
}
?>