...
<?php
  if (isset($_POST['submit'])) {
    # code...
    $sql = "SELECT * FROM `tblotherinformation` WHERE `EMPLOYEEID` ='".$_SESSION['EMPLOYEEID']."'";
    $mydb->setQuery($sql);
...
<form method="POST" action="index.php?tab=otherinfo">
...
<table class="table   table-bordered" style="font-size:12px" cellspacing="0">
   <thead style="text-align:center">
    <tr >  
       <td rowspan="2">Refree/Guarantor (Fill in names off three persons, other than relatives, who can testify to your character).</td>
       <td colspan="2" width="200px" >Employee's Compensation: Nominatin of Beneficiaries</td>
    </tr>
  </thead> 
  <tbody>
   <?php  
    ...
    ?>
   <tr>
     <td><textarea class="form-control" rows="9" name="SKILLS"><?php echo isset($otherres->SKILLS) ? $otherres->SKILLS : ""; ?></textarea></td>
     <td><textarea class="form-control" rows="9" name="NDRA" ><?php echo isset($otherres->NDRA) ? $otherres->NDRA: ""; ?></textarea></td>
   </tr>
   <tr>
     <td colspan="2">
